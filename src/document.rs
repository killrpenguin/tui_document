...
use crate::{RopeyResult, search::SearchIter};
...
impl<'a> Document<'a> {
    ...
    /// Creates a `Document` from a string or string slice.
    /// # Example
    /// ```no_run
    /// use tui_document::Document;
    ...
    ///     let mut file = std::fs::File::open("foo.txt")?;
    ///     let doc_str = Document::from_reader(file);
    ///     Ok(())
    /// }
    /// ```
    ///
    ...
    /// Writes the contents of the underlying [`Rope`](https://docs.rs/ropey/latest/ropey/index.html) to a writer.
    ///
    /// When more precise control over IO behavior, buffering, etc. is
    /// desired, you should handle IO yourself and use the `Chunks`
    /// iterator to iterate through the [`Rope`](https://docs.rs/ropey/latest/ropey/index.html)'s contents.
    /// # Example
    /// ```no_run
    /// use tui_document::Document;
    ///
    /// fn main() -> std::io::Result<()> {
    ///     let doc = Document::from_str("This is a string slice.");
    ///     let mut file = std::fs::File::create("foo.txt")?;
    ///     let _ = doc.write_file(file)?;
    ///     Ok(())
    /// }
    /// ```
    ///
    ...
    /// Total number of bytes in the underlying [`Rope`](https://docs.rs/ropey/latest/ropey/index.html).
    pub fn len_bytes(&self) -> usize {
        self.rope.len_bytes()
    }
    /// Total number of chars in the underlying [`Rope`](https://docs.rs/ropey/latest/ropey/index.html).
    pub fn len_chars(&self) -> usize {
        self.rope.len_chars()
    }
    /// Total number of lines in the underlying [`Rope`](https://docs.rs/ropey/latest/ropey/index.html).
    ...
    /// An iterator over the underlying [`Rope`](https://docs.rs/ropey/latest/ropey/index.html)'s chars starting at a given position.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// An iterator over a [`Rope`](https://docs.rs/ropey/latest/ropey/index.html)'s lines at a given position.
    ///
    ...
    /// An iterator over a [`Rope`](https://docs.rs/ropey/latest/ropey/index.html)'s lines.
    ///
    ...
    /// An iterator over a [`Rope`](https://docs.rs/ropey/latest/ropey/index.html)'s contiguous `str` chunks.
    ///
    /// Internally, each [`Rope`](https://docs.rs/ropey/latest/ropey/index.html) stores text as a segemented collection of utf8
    ...
    /// An iterator over sequences of the [`Rope`](https://docs.rs/ropey/latest/ropey/index.html) that match a given pattern using
    /// the Boyor-Moore search algorithm.
    ///
    ///
    /// # Example
    ///
    ...
    /// use tui_document::Document;
    ///
    /// let doc = Document::from_str("adcabcafg");
    /// let mut search_iter = doc.search_rope("abc");
    ///
    /// assert_eq!(search_iter.next(), Some((3, 6)));
    /// assert_eq!(search_iter.next(), None);
    /// ```
    pub fn search_rope<'b>(&'b self, pattern: &'b str) -> SearchIter<'b> {
        SearchIter::from_rope_slice(self.rope.slice(..), pattern)
    }
    /// Gets an immutable slice of the underlying Rope based on the current visible range..
    ///
    /// # Example
    ///
    /// ```
    ...
    /// Gets an immutable slice of the underlying Rope.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// The given position is represented as a pair of row/col coordinates.
    ///
    /// Does not panic.
    ///
    ///
    ...
    /// Insert a string or string slice at the given position.
    /// The given position is represented as a pair of row/col coordinates.
    ///
    /// Does not panic.
    ///
    ...
    /// indices.
    ///
    /// Runs in O(M + log N) time, where N is the length of the [`Rope`](https://docs.rs/ropey/latest/ropey/index.html) and M
    ...
    ///
    /// let mut doc = Document::from_str("Hello, world!");
    /// assert_eq!(doc.name("example.txt"), Some(()));
    /// assert_eq!(doc.name("cant_change_me_now.txt"), None);
    /// assert_eq!(doc.rename("use_rename_to_overwrite_me.txt"), ());    
    /// ```
    pub fn name<S: AsRef<str>>(&mut self, path: S) -> Option<()> {
